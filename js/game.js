class Game{
    ...
    play(){
                ...
                if (keyIsDown(LEFT_ARROW) && player.index !== null) {
                 ...
                 if (frameCount % 20 === 0) {
                    gamepieces = createSprite(random(100, 1000), 0, 100, 100);
                     gamepieces.velocityY = 6;
                     var rand = Math.round(random(1,2));
                     switch(rand){
                         case 1: gamepieces.addImage("bat",bat_img);
                         break;
                         case 2: gamepieces.addImage("baseball", baseball_img);
                         break;
                    
                     }
                     
                     gameGroup.add(gamepieces);
                     gamepieces.scale=0.1
                 }
                 ...
                 if(frameCount % 40 === 0){
                  ...
                  if (player.index !== null) {
                      for (var i = 0; i < gameGroup.length; i++) {
                          if (gameGroup.get(i).isTouching(players)) {
                              gameGroup.get(i).destroy();
